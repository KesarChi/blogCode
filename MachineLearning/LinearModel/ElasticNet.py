    ...
    def fit(self, X, Y):
        ...
        for t in range(self.maxIter):
            for i, w in enumerate(self.weights):
                grad = np.sum(X[:, i] * (np.sum(X * self.weights, axis=1) + self.bias - Y)) / len(X[0]) + self.alpha * self.rho * np.sign(w) + self.alpha * (1 - self.rho) * w
...
if __name__ == "__main__":
    ...
    model = ElasticNetRegression(LR=1e-6, maxIter=3000, alpha=1, rho=0.5)
    model.fit(X, Y)
